...
const FEATURE_TITLE: string = "No 3-way invoice-to-PO-to-receipt matching workflow";
  ...
  try {
    ...
    if (typeof db.query === 'function') {
      await db.query(
        ...
        ['ai_procurement_management', '3-way-invoice-po-receipt', JSON.stringify(input), JSON.stringify(output)]
      );
    }
  } catch (err) { /* ignore */ }
}
...
router.get('/health', (_req: Request, res: Response) => {
  res.json({ feature: '3-way-invoice-po-receipt', status: 'ok', version: 'v0', kind: 'gap' });
});
...
router.post('/run', async (req: Request, res: Response) => {
  try {
    ...
    const output = {
      feature: '3-way-invoice-po-receipt',
      title: FEATURE_TITLE,
    ...
    console.error('[gap-3-way-invoice-po-receipt] error', err);
