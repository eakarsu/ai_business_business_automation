...
const FEATURE_TITLE: string = "No e-signature workflow for contracts";
  ...
  try {
    ...
    if (typeof db.query === 'function') {
      await db.query(
        ...
        ['ai_procurement_management', 'e-signature-workflow-contracts', JSON.stringify(input), JSON.stringify(output)]
      );
    }
  } catch (err) { /* ignore */ }
}
...
router.get('/health', (_req: Request, res: Response) => {
  res.json({ feature: 'e-signature-workflow-contracts', status: 'ok', version: 'v0', kind: 'gap' });
});
...
router.post('/run', async (req: Request, res: Response) => {
  try {
    ...
    const output = {
      feature: 'e-signature-workflow-contracts',
      title: FEATURE_TITLE,
    ...
    console.error('[gap-e-signature-workflow-contracts] error', err);
